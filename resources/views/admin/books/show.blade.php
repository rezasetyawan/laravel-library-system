    ...
    <main class="p-6">
                ...
                <div class="col-span-9">
                    ...
                    <div class="mt-4">
                        ...
                        <x-textarea id="description" name="description" type="text" class="mt-1 block w-full"
                            required autofocus rows="5">{{ old('description', $book->description) }}</x-textarea>
                    ...
                    <div class="mt-4">
                        ...
                        <x-text-input id="page_count" name="page_count" type="text" class="mt-1 block w-full"
                            required autofocus value="{{ old('page_count', $book->page_count) }}" />
        ...
        <section class="space-y-6">
            <x-danger-button x-data=""
                x-on:click.prevent="$dispatch('open-modal', 'confirm-book-deletion')">{{ __('Delete Account') }}</x-danger-button>

            <x-modal name="confirm-book-deletion" :show="$errors->bookDeletion->isNotEmpty()" focusable>
                <form method="post" action="{{ route('books.destroy', ['book' => $book]) }}?softDelete=false"
                    class="p-6">
                    @csrf
                    @method('delete')
